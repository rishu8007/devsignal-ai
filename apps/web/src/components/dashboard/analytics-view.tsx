...
export function AnalyticsView({ active, onNavigate }: { active: boolean; onNavigate?: (tab: "drafts") => void }) {
  ...
  return (
    <section className="mt-8 space-y-6" aria-labelledby="analytics-heading">
      ...
      {!loading && !error && data && <div className="space-y-6">
        ...
        <section className="rounded-xl border border-indigo-200 bg-indigo-50 p-4" aria-labelledby="quality-insights-heading"><div className="flex flex-wrap items-baseline justify-between gap-3"><h3 id="quality-insights-heading" className="font-semibold">Draft quality insights</h3><span className="text-lg font-bold text-indigo-700">{data.reviewInsights.averageScore === null ? "Not assessed" : `${data.reviewInsights.averageScore.toFixed(1)}/100`}</span></div><p className="mt-1 text-xs text-slate-600">Advisory draft-quality heuristic, rubric v{data.reviewInsights.rubricVersion}; it is not a prediction of engagement and does not approve or publish drafts.</p><p className="mt-3 text-sm text-slate-700">Population: {data.reviewInsights.population} draft variations created in this reporting period. Coverage: {data.reviewInsights.assessed} assessed, {data.reviewInsights.unassessed} unassessed, {data.reviewInsights.outdated} outdated.{data.reviewInsights.truncated ? ` Results capped at ${data.reviewInsights.populationLimit} generations.` : ""}{data.reviewInsights.reviewQueryTruncated ? " Review history was capped while selecting the latest applicable review." : ""}</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><div><p className="text-sm font-medium text-slate-700">Finding breakdown</p><p className="text-sm text-slate-600">Severity: {Object.entries(data.reviewInsights.severities).map(([key, value]) => `${key} ${value}`).join(", ") || "No current findings"}</p><p className="text-sm text-slate-600">Category: {Object.entries(data.reviewInsights.categories).map(([key, value]) => `${key} ${value}`).join(", ") || "No current findings"}</p></div><div><p className="text-sm font-medium text-slate-700">Common improvements</p>{data.reviewInsights.suggestions.length ? <ul className="list-disc pl-5 text-sm text-slate-600">{data.reviewInsights.suggestions.map((item) => <li key={item.text}>{item.text} ({item.count})</li>)}</ul> : <p className="text-sm text-slate-600">No current suggestions.</p>}</div></div>{onNavigate && <button type="button" onClick={() => onNavigate("drafts")} className="mt-3 text-sm font-semibold text-indigo-700 underline">Open drafts and review</button>}</section>
