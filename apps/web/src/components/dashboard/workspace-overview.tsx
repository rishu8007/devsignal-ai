...
export function WorkspaceOverview({
  ...
  const statistics = [
    ...
    { label: "Scheduled", value: scheduled === null ? "—" : String(scheduled), icon: CalendarDays, tone: "text-amber-600 bg-amber-50" },
  ];
  return (
    <section
      ...
      <div className="grid grid-cols-3 gap-3 sm:max-w-md lg:w-[390px]">
        {statistics.map(({ icon: Icon, label, tone, value }) => (
          <div
            ...
            title={
              label === "Scheduled"
                ? "Approved variations with a planned date, including past plans until removed."
                : undefined
            }
